...
class TestUserAgent:
    ...
    @pytest.mark.parametrize("test_data", test_user_agent_params)
    def test_user_agent(self, test_data):
        url = "https://playground.learnqa.ru/ajax/api/user_agent_check"
        user_agent = test_data["user_agent"]
        user_agent_id = test_data["param_id"]
        headers_data = {"User-Agent": user_agent}
        response_fields_to_check = {"platform", "browser", "device"}
        response = requests.get(url, headers=headers_data, verify=False)
        response_json = response.json()
        for field in response_fields_to_check:
            assert response_json[field] == test_data[field], \
                (f"Actual '{field}' value '{response_json[field]}' differs from '{test_data[field]}' expected. "
                 f"Problem with User-Agent '{user_agent_id}': '{user_agent}'")
